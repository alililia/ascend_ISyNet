...
│   ├── run_standalone_train_.sh     # training script on single 
│   └── run_distributed_train.sh        # distributed training script on multiple s
├── ISyNet
...
```bash
# training on single 
  bash run_standalone_train_.sh  [DATA_PATH] [CONFIG_PATH]
# training on multiple s
  bash run_distributed_train_.sh [DATA_PATH] [CONFIG_PATH]
# training on multiple Ascends
  bash run_distributed_train.sh [RANK_TABLE] [DATA_PATH] [CONFIG_PATH]
```

> checkpoints will be saved in the ./train/output folder (single )
./train_parallel/output/ folder (multiple s)
...
bash run_eval_.sh [DATA_PATH] [CHECKPOINT_PATH] [CONFIG_PATH]
```
